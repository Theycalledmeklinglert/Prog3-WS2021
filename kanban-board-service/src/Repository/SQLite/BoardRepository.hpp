...
class BoardRepository : public RepositoryIf {
  ...
  public:
    ...
    static int queryCallbackColumn(void *data, int numberOfColumns, char **fieldValues, char **columnNames); // statt static geht auch Free Function; Dann aber auch Aufruf ohne BoardRepository::
    static int queryCallbackItems(void *data, int numberOfColumns, char **fieldValues, char **columnNames);
